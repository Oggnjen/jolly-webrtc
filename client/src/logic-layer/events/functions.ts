...
export const dispatchAcceptOfferAndSendAnswer = (memberId: string, payload: string, senderName: string) => {
...
export const dispatchAcceptIceCandidate = (memberId: string, payload: string) => {
...
addTypedEventListener<{ memberId: string; payload: string }>('accept-answer', async (e) => {
...
addTypedEventListener<{ memberId: string; payload: string }>('accept-ice', async (e) => {
  const { memberId } = e.detail;
  const { payload } = e.detail;
  const { identifier } = userStore.getState();
  const { members } = callStore.getState();
